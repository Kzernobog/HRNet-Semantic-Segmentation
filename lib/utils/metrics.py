...
class Evaluator(object):
    ...
    def class_IOU(self,class_num):
        intersection = self.confusion_matrix[class_num,class_num]
        union = np.sum(self.confusion_matrix[2,:]) + np.sum(self.confusion_matrix[:,2]) - intersection
        if union != 0:
